...
class EditChallengesModal extends React.Component {
  ...
  componentDidMount() {
    ...
    const {chal} = this.props;
    this.setState({
      id: chal.categoryId,
      name: chal.name,
      hashtag: chal.hashtagName,
      sponsor: chal.sponsorId
    })
  }
  ...
  handleSubmit = (e) => {
    e.preventDefault();
    const { name, hashtag, imageURL, sponsor, id } = this.state;
    if (id === '' || name === '' || hashtag === '') return;
    this.setState(prevState => ({
      isMakingRequest: !prevState.isMakingRequest
    }))
    this.props.editChallenge({ name, categoryId: id, hashtag, imageURL, sponsorId: sponsor }, this.props.chal._id).then(res => {
      this.setState(prevState => ({
        isMakingRequest: !prevState.isMakingRequest
      }))
      if (res === "success") {
...
export default connect(mapStateToProps, mapDispatchToProps)(EditChallengesModal);
